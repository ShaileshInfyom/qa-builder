...
class AuthController extends AppBaseController
{
    ...
    public function loginWithOTP(LoginAPIRequest $request): JsonResponse
    {
        ...
        $data['message'] = 'Please check your email for OTP.';
     ...
     * @return bool
     */
    public function sendOTPEmail($user, string $code): bool
    {
        $user->update([
        ...
        // mail send code
        $data['otp'] = $code;
        $data['expireTime'] = User::OTP['expire_time'];
        Mail::to($user->email)
            ->send(new MailService('emails.OTP',
            'Login OTP',
            $data));
